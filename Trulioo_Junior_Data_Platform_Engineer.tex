...
\item \bodytext{Integrated \textbf{ML models} for entity resolution matching clinic records across sources and semantic enrichment of call transcripts, applying \textbf{cosine similarity} and threshold-based matching to improve data quality}
\item \bodytext{Optimized database performance through indexing strategies on vector columns using \textbf{HNSW algorithm}, query tuning for similarity searches, and monitoring data pipeline health with custom logging and alerting}
\end{itemize}
...
\item \bodytext{Implemented \textbf{similarity search} using \textbf{cosine distance} metrics with metadata filtering on document type and timestamp, achieving response times under \textbf{200ms} for queries across knowledge base}
\item \bodytext{Designed data ingestion workflow extracting text from PDFs and web pages, applying text splitters with \textbf{500-token chunks} and \textbf{50-token overlap}, then generating embeddings in batches for efficient processing}
\item \bodytext{Developed retrieval evaluation framework comparing different chunking strategies and embedding models, measuring precision and recall to optimize search relevance for domain-specific queries}
\end{itemize}

\vspace{5pt}\cvevent{ETL Orchestration Platform with Airflow}{Python, Apache Airflow, PostgreSQL, Docker}{}{}
\begin{itemize}
\item \bodytext{Designed and deployed \textbf{Apache Airflow} DAGs orchestrating multi-stage ETL workflows, extracting data from \textbf{CSV sources} and external APIs, transforming with \textbf{pandas}, and loading into \textbf{PostgreSQL}}
\item \bodytext{Implemented task dependencies and retry logic handling upstream failures gracefully, using Airflow sensors to wait for data availability before triggering downstream transformations}
\item \bodytext{Built data validation steps using \textbf{Great Expectations} framework, defining data quality checks for schema validation, null checks, and statistical distribution tests}
...
\begin{justify}
\bodytext{Data platform engineer passionate about building scalable data systems that bridge traditional databases and modern ML workflows. Hands-on experience architecting production ETL pipelines, designing data models for SQL and vector databases, and integrating embeddings for semantic search. Built multi-tenant data platforms processing document ingestion, transformation, and retrieval using PostgreSQL, pgvector, and Python. Strong foundation in data quality monitoring, workflow orchestration, and database optimization from both academic training and real-world implementation. Eager to apply data engineering skills to identity verification challenges while learning graph databases, streaming systems, and advanced MLOps practices from experienced engineers.}
\end{justify}
\endgroup
...
\bodytext{Apache Airflow, Pandas, NumPy, LangChain, Data Modeling, Pipeline Orchestration, ETL Workflows, Data Transformation}

\cvsubsection{Databases \& Data Stores}
\bodytext{PostgreSQL, pgvector, Vector Databases, NoSQL, Pinecone, Database Design, Query Optimization, Indexing Strategies}

\cvsubsection{Cloud \& Infrastructure}
\bodytext{AWS (S3, Lambda), Vercel, Docker, Supabase, Serverless Architecture, CI/CD Pipelines}

\cvsubsection{Machine Learning \& AI}
\bodytext{Embeddings, Semantic Search, RAG Systems, Entity Resolution, scikit-learn, Feature Engineering, Statistical Modeling}

\cvsubsection{Data Tools \& Frameworks}
\bodytext{FastAPI, Jupyter, Great Expectations, Git, REST APIs, Webhooks, Real-time Data Processing}

\cvsubsection{Visualization \& Analytics}
\bodytext{Tableau, Data Dashboards, KPI Development, Statistical Analysis, Hypothesis Testing}
